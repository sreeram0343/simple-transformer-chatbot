...
class SidebarPanel(ctk.CTkFrame):
    ...
    def __init__(self, parent, on_clear_callback, on_reload_callback):
        ...
        self.logo_label = ctk.CTkLabel(
            self, 
            text="⚡ NEURAL ORCHESTRATOR", 
            font=st.FONT_TITLE,
            text_color=st.ACCENT_PRIMARY
        )
        self.logo_label.grid(row=0, column=0, padx=20, pady=(40, 5), sticky="w")
        
        self.version_label = ctk.CTkLabel(
            self,
            text="CORE ENGINE :: v1.0.4-BETA",
            font=st.FONT_CODE,
        ...
        self.status_card = ctk.CTkFrame(self, fg_color=st.BG_CARD, border_width=1, border_color=st.ACCENT_SECONDARY, corner_radius=st.CORNER_RADIUS_MD)
        ...
        self.status_dot.grid(row=0, column=0, padx=(15, 10), pady=18)
        ...
        self.status_text.grid(row=0, column=1, padx=(0, 15), pady=18, sticky="w")
        ...
        ctk.CTkLabel(
            self.stats_card, 
            text="NEURAL SPECIFICATIONS", 
            font=st.FONT_CODE,
            text_color=st.TEXT_MUTED
        ).grid(row=0, column=0, padx=15, pady=(15, 8), sticky="w")
        
        # Info Fields
        self.info_device = self._create_info_row(self.stats_card, "ACTIVE NODE:", "DETECTING...", 1)
        self.info_vocab = self._create_info_row(self.stats_card, "VOCAB DENSITY:", "N/A", 2)
        self.info_layers = self._create_info_row(self.stats_card, "SYNAPTIC LAYERS:", "2 BLOCKS", 3)
        self.info_embed = self._create_info_row(self.stats_card, "EMBED VECTORS:", "32 DIM", 4)
        ...
        self.actions_frame.grid(row=6, column=0, padx=15, pady=30, sticky="ew")
        ...
        self.reload_btn = ctk.CTkButton(
            ...
            border_width=1,
            border_color=st.ACCENT_PRIMARY,
            hover_color=st.BG_PANEL,
            text_color=st.TEXT_MAIN,
            corner_radius=st.CORNER_RADIUS_SM,
            command=self.on_reload_callback
        )
        self.reload_btn.grid(row=0, column=0, padx=0, pady=6, sticky="ew")
        ...
        self.clear_btn.grid(row=1, column=0, padx=0, pady=6, sticky="ew")
    ...
    def update_model_info(self, device: str, vocab_size: int, layers: int, embed_dim: int, status: str):
        ...
        if status == "ready":
            ...
            self.status_text.configure(text="NODE READY", text_color=st.TEXT_MAIN)
        elif status == "training":
            self.status_dot.configure(fg_color=st.COLOR_WARNING)
            self.status_text.configure(text="SYNTHESIZING...", text_color=st.COLOR_WARNING)
        else:
            self.status_dot.configure(fg_color=st.COLOR_ERROR)
            self.status_text.configure(text="NODE OFFLINE", text_color=st.TEXT_MUTED)
